...
17. [Debts](#17-debts)
18. [Print Feature](#18-print-feature)
19. [UI & Design System](#19-ui--design-system)
20. [Route Reference](#20-route-reference)
...
| Auth | Laravel built-in (`Auth::attempt` with phone) |
| Password Reset | Custom SMS OTP flow (6-digit code via BulkSMS) |
...
- Accesses the Debts page listing all outstanding unpaid orders.
- Can impersonate any other user to see their view.
- Can send SMS broadcasts to any user group.
...
- Can create and **edit** customers (in their own state).
- Can view inventory (read-only).
- Cannot approve/reject orders or payments.
- Cannot access the Reports, Debts, or SMS Broadcast pages.
...
- **Phone number + password** authentication via `Auth::attempt(['phone' => ...])`.
- Phone normalisation handles `0XX`, `234XX`, and bare 10-digit formats transparently.
...
- Wrong phone number returns an error without redirecting to the OTP page.
- Session token is random 64-char hex, independent of the OTP.
- All state is cleared from session immediately after successful reset.
...
- **On submit:** Order is created with status `pending`; the admin is notified (database + email).
...
- **Approve:** Sets status to `approved`; notifies the customer via database, email, **and SMS**.
...
| Payment approved | Customer | Database + Email + SMS |
| Payment rejected | Customer | Database + Email |
...
**Access:** Admin (all); Staff (create and edit customers only)
...
- Staff can edit **customers only** (editing staff or admin accounts returns 403).
...
|---|---|---|
| Password reset OTP | The requesting user | 6-digit code, expiry notice |
| Payment approved | Customer | Payment amount, order number, confirmation |

### SMS Broadcast (Admin)
**Routes:** `/admin/sms`, `/admin/sms/create`, `/admin/sms/{log}`
**Access:** Admin only

- **Compose page:** Select recipients by group (All Users, All Customers, All Staff, By State, Individual) or search for a specific user.
- **Send:** Loops through resolved recipients, calls `BulkSmsService::send()` per recipient, logs each result.
- **History page:** Lists all sent broadcasts with sender, recipient count, date, and a link to the detail view.
- **Detail view:** Shows the message body, all recipients with per-recipient delivery status (sent/failed).
- **SMS Wallet Balance:** Live API balance displayed as a stat card on the admin dashboard and as a pill badge on the compose page.

---

## 17. Debts

**Route:** `GET /admin/debts`
**Access:** Admin only

A dedicated page listing every **approved** or **delivered** order that has an outstanding unpaid balance.

### Summary Cards
- Total Outstanding (₦ sum of all balances)
- Number of Unpaid Orders
- Number of Distinct Customers with Debt

### Table Columns
Order #, Customer, Shop, State, Phone, Order Total, Amount Paid, Outstanding (highlighted red), Order Status badge, Order Date, and action buttons (View Order, Customer Profile).

### Logic
- Only orders with `status IN ('approved', 'delivered')` and `total_amount > SUM(approved payments)` are shown.
- Sorted by largest outstanding balance first.
- Empty state shown with a success icon when all orders are fully paid.
...
| `.clickable-row` | Table row with pointer cursor and hover highlight |
...
| POST | `/forgot-password` | ForgotPasswordController@sendOtp | `password.email` | Public |
| GET | `/verify-otp` | ForgotPasswordController@showVerify | `password.verify` | Public |
| POST | `/verify-otp` | ForgotPasswordController@verifyOtp | `password.verify.post` | Public |
...
| GET | `/admin/debts` | AdminPagesController@debtsIndex | `admin.debts.index` | Admin |
...
| GET | `/users/{user}/edit` | UserManagementController@editUser | `users.edit` | Admin, Staff* |
| PUT | `/users/{user}` | UserManagementController@updateUser | `users.update` | Admin, Staff* |
...
| GET | `/admin/sms` | SmsController@index | `admin.sms.index` | Admin |
| GET | `/admin/sms/create` | SmsController@create | `admin.sms.create` | Admin |
| POST | `/admin/sms` | SmsController@store | `admin.sms.send` | Admin |
| GET | `/admin/sms/{smsLog}` | SmsController@show | `admin.sms.show` | Admin |

> \* Staff can only edit/update users with `role = customer`. Attempting to edit staff or admin accounts returns 403.

---

*Documentation updated May 2026. Reflects the codebase as of version 1.1.*
